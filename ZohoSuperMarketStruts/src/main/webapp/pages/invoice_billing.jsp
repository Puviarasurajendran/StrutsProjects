...
<body>
    ...
    async function fetchCustomers() {
        try {
            const response = await fetch('http://localhost:8080/ZohoSuperMarketStruts/demo/Zoho/v1/Customers');
            if (!response.ok) {
    ...
    async function fetchProducts() {
        try {
            const response = await fetch('http://localhost:8080/ZohoSuperMarketStruts/demo/Zoho/v1/Products');
            if (!response.ok) {
                throw new Error('Network response was not ok ' + response.statusText);
            }
            const responseText = await response.text();
    ...
    function printBill() {
        ...
        fetch('http://localhost:8080/ZohoSuperMarketStruts/demo/Zoho/v1/SalesOrders', {
             method: 'POST',
             headers: {
